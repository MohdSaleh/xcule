...
const io = new Server(EXPRESS_SOCKET_PORT, {  cors: {
  origin: '*',
  methods: ["GET", "POST"]
}});
...
const getLiveFeedData = ({data, socket}, callback) => {
 ...
 const tf = (['1D', '1W', '1M']).includes(data.tf)? data.tf.replace('1', '') : data.tf

  console.log("GET LC OF: ", symbol, tf)
  socketChart[data.uid].setMarket(symbol, { // Set the market
    timeframe:tf,
  });
  // Call the onUpdate function without a promise
  socketChart[data.uid].onUpdate(() => { // When price changes
    if (!socketChart[data.uid].periods[0]) return console.error(new Error("No data"));
    callback(socketChart[data.uid].periods[0]);
  });

  socketChart[data.uid].onError((...err) => { // Listen for errors (can avoid crash)
 ...
 const exitRoomIfNoUser = async (data) =>{
  let sks = await io.of("/").in(data.uid).fetchSockets()
  if(!sks.length){
...
//  let replySocketClient = []
//  let replySocketChart = []
...
//         console.log("OLD USER + ALREADY IN ", data.uid);
//       } else{
//           RnewClientSessionInit(data)
//           socket.join(data.uid);
//           console.log("NEW-USER JOINING ROOM", data.uid);
//           //SENDING DATA
//           RgetLiveFeedData({data, socket}, cb => {
//             // console.log("LC-NEW("+ data.uid +"): ",cb.time,  cb.close) 
...
// const RgetLiveFeedData = async ({data, socket}, callback) => {
// symbol = data.ticker
// tf = (['1D', '1W', '1M']).includes(data.tf)? data.tf.replace('1', '') : data.tf
// uid = data.uid

// console.log("GET LC OF: ", symbol, tf)
// replySocketChart[uid].setMarket(symbol, { // Set the market
//   timeframe:tf,
// });
// // Call the onUpdate function without a promise
// replySocketChart[uid].onUpdate(() => { // When price changes
//   if (!replySocketChart[uid].periods[0]) return console.error(new Error("No data"));
//   callback(replySocketChart[uid].periods[0]);
// });

// replySocketChart[uid].onError((...err) => { // Listen for errors (can avoid crash)
//   replySocketChart[uid].end()
//   return console.error(new Error("DATA Error"));
// });
...
// const RnewClientSessionInit=(data)=>{
//   replySocketClient[data.uid] = new TradingView.Client(); // Creates a websocket client
//   replySocketChart[data.uid] = new replySocketClient[data.uid].Session.Chart(); // Init a Chart session
//   replySocketChart[data.uid].setTimezone('Asia/Kolkata')
// }

// const RexitRoomIfNoUser = async (data) =>{
// sks = await io.of("/").in(data.uid).fetchSockets()
// if(!sks.length){
//   console.log("NO USERS & CLOSING THE ROOM ", data.uid)
//   replySocketClient[data.uid].end()
// }
// }
 ...
 app.post("/signIn", jsonParser, async (req, res) => {
  ...
  try {
    ...
    // res.status(200).json({"message":"haloooo"});
...
server.listen(() => console.error('WebSocket listening at', EXPRESS_SOCKET_PORT));
app.listen(EXPRESS_PORT, console.log("Http server listening at", EXPRESS_PORT));